...
public class XCMPMsg extends XCMP {
    ...
    void receive() throws IOException, InterruptedException {
        ...
        while (x) {
            ...
            /* if it's ACK */
            if (OPCODE == (byte) 0x0C) {
                System.out.print("OPCODE " + Integer.toHexString(OPCODE));
        ...
        switch (data[10]) {
            case (byte) 0xB4:
                if (data[11] == (byte) 0x10) {
                    ByteBuffer batteryLevelBytes = ByteBuffer.allocate(4).put(new byte[]{data[12], data[13]});
                    boolean critical = data[12] == 0x1;
                    boolean recondition = data[13] == (byte) 0xFF;

                    if (recondition) {
                        System.out.println("Recharge the battery immediately!");
                    }

                    int batteryLevel = data[13];

                    System.out.println(String.format(" Battery Level%s %i", critical ? "(!critical)" : "", batteryLevel));
                }
